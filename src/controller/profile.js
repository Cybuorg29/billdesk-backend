...
exports.setupProfile = async (req, res) => {
  try {
    ...
    const { name, gstin, phone, email, adress, state, activities, pincode } = user
    const { Accountname, no, isfc, bankName, branch } = bankDetail
    if (!name || !gstin || !phone || !email || !adress || !state || !activities || !pincode || !Accountname || !no || !isfc || !bankName || !branch) {
      return res.status(200).json({ code: 400, message: 'form not full filled' })
    }
    console.log(image)

    const id = await UserModel.convertToken(token);
    const updateProfile = await profileModel.findOneAndUpdate({ id: id }, { $set: { name: name, gstin: gstin, phone: phone, adress: adress, email: email, image: image, state: state, pincode: pincode, activities: activities, } })
