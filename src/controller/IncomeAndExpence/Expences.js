...
exports.createExpence = async (req, res) => {
    try {
        ...
        const { data } = req.body
        const { title, amount, category, token, date, uid, E_id } = data;
        ...
        const pushExpences = await expencesModel.create({ title, amount, category, id, date: date, uid: uid, E_id: E_id })
